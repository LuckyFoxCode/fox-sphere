...
handler come out of the **same object**. That is the whole point - a route cannot be
mounted without appearing in the spec, and the spec cannot describe a route that is not
mounted.
...
Then mount it once in `apps/api/src/app.ts`:
...
`method` is `"get" | "post" | "put" | "patch" | "delete"`. Everything else is the same
shape; only what you validate changes.
...
- **Schemas live in `packages/shared-schemas`**, named with `.openapi("Name")` so they land
  in `components.schemas` and orval emits one type module per schema. An inline
  `z.object({...})` works but generates an anonymous inline type - only for a response
  nothing else needs.
- **Document every status code you actually return**, the 4xx included. `responses` with
  no `schema` documents a body-less answer (204) or an error shape the handler throws.
...
```bash
pnpm --filter api openapi:dump     # rewrites apps/api/openapi.json
pnpm --filter admin gen:api        # regenerates the admin client from that file
```
