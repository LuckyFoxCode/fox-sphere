<script setup lang="ts">
import { ref } from 'vue';

const dynamicColor = ref('#1E90FF');
</script>

<template>
  <li class="bg-text-main/80 relative rounded-xl px-1.5 pt-4">
    <div
      class="bg-bg/90 absolute -top-3 left-1.5 flex items-center gap-x-0.5 rounded-xl border-2 px-1 py-0.5 text-sm"
      :style="{ borderColor: dynamicColor }"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="16"
        height="16"
        viewBox="0 0 24 24"
        class="text-event-rose"
      >
        <g
          fill="none"
          fill-rule="evenodd"
        >
          <path
            d="m12.593 23.258l-.011.002l-.071.035l-.02.004l-.014-.004l-.071-.035q-.016-.005-.024.005l-.004.01l-.017.428l.005.02l.01.013l.104.074l.015.004l.012-.004l.104-.074l.012-.016l.004-.017l-.017-.427q-.004-.016-.017-.018m.265-.113l-.013.002l-.185.093l-.01.01l-.003.011l.018.43l.005.012l.008.007l.201.093q.019.005.029-.008l.004-.014l-.034-.614q-.005-.018-.02-.022m-.715.002a.02.02 0 0 0-.027.006l-.006.014l-.034.614q.001.018.017.024l.015-.002l.201-.093l.01-.008l.004-.011l.017-.43l-.003-.012l-.01-.01z"
          />
          <path
            fill="currentColor"
            d="M17 3a2 2 0 0 1 1.492.668l.108.132l3.704 4.939a2 2 0 0 1-.012 2.416l-.108.13l-9.259 10.184a1.25 1.25 0 0 1-1.753.096l-.097-.096l-9.259-10.185a2 2 0 0 1-.215-2.407l.095-.138L5.4 3.8a2 2 0 0 1 1.43-.793L7 3zm-2.477 8H9.477L12 17.307zm5.217 0h-3.063l-2.406 6.015zM7.323 11H4.261l5.468 6.015zm5.059-6h-.764l-2 4h4.764zM17 5h-2.382l2 4H20zM9.382 5H7L4 9h3.382z"
          />
        </g>
      </svg>
      <span
        class="text-event-cyan"
        :style="{ color: dynamicColor }"
        >LuckyFoxCode</span
      >
    </div>
    <p class="text-bg/80 text-sm">
      Lorem ipsum, dolor sit amet consectetur adipisicing elit. Nulla, eveniet!
    </p>
  </li>
</template>
